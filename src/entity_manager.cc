...
EntityManager::EntityManager(FileSystem* fs) { this->fs = fs; }

// split_request takes a request "api/shoes/1" and returns tokenized array
// [shoes,1] it accounts for nested path from location
std::vector<std::string> EntityManager::split_request(
    const std::string& path, const std::string& request_target) {
...
void EntityManager::set_response(
    int status_code,
    ...
    if (status_code == 400) {
        ...
        response.body() =
            "<html><head><title>400 Bad Request</title></head><body><p>400 Bad "
            "Request</p></body></html>";
    }
    ...
    // 204 No content is a success status code that indicates that a request has
...
bool EntityManager::insert(
    const std::string& path, const std::string& root,
    const boost::beast::http::request<boost::beast::http::string_body>& request,
    boost::beast::http::response<boost::beast::http::string_body>& response,
    int optional_insert_id) {
    std::string request_target(request.target().data(),
                               request.target().size());
    std::vector<std::string> split = split_request(path, request_target);

    // These conditions would signify invalid parameters if optional_insert_id
    // not passed hence we return a bad request
    if ((optional_insert_id == -1) &&
        (split.size() >= 2 || split.size() == 0)) {
    ...
    std::string body =
        "{EXAMPLE PAYLOAD: this codebase does not store request body into "
        "boost::beast request}";
    ...
    if (fs->exists_directory(directory_path)) {
        if (optional_insert_id == -1) {
            ...
            std::string attempt_insert_file_path =
                directory_path + "/" + std::to_string(attempt_insert_id);
            while (fs->exists_file(attempt_insert_file_path)) {
                attempt_insert_id += 1;
                attempt_insert_file_path =
                    directory_path + "/" + std::to_string(attempt_insert_id);
            }
            // Once the while loop is broken we know the place of insertion is
            // not occupied
        ...
        if (list_contents.size() != 0) {
            list_contents = list_contents.substr(0, list_contents.size() - 1) +
                            "," + std::to_string(id_insert) + "]";
        }
        ...
        if (optional_insert_id == -1) {
            body =
                "{EXAMPLE PAYLOAD: this codebase does not store request body "
                "into boost::beast request}";
        } else {
            body =
                "{EXAMPLE UPDATED PAYLOAD: this codebase does not store "
                "request body into boost::beast request}";
        }
        fs->write_file(list_path, list_contents);
        ...
        if (fs->create_directory(directory_path)) {
            ...
            if (optional_insert_id == -1) {
                body =
                    "{EXAMPLE PAYLOAD: this codebase does not store request "
                    "body into boost::beast request}";
            } else {
                body =
                    "{EXAMPLE UPDATED PAYLOAD: this codebase does not store "
                    "request body into boost::beast request}";
...
bool EntityManager::get_entity(
    ...
    // if there are invalid number of arguments to GET or LIST requests,
    ...
    if (split.size() == 2) {
        ...
        if (fs->exists_file(file_path)) {
            if (!fs->delete_file(file_path)) {
                Logger::log_info(
                    "Filesystem error occurred while removing the file");
                return false;
            }
...
bool EntityManager::delete_(
    ...
    if (split.size() == 2) {
        ...
        if (fs->exists_file(file_path)) {
            if (!fs->delete_file(file_path)) {
                Logger::log_info(
                    "Error occurred while initially removing the file in "
                    "EntityManager::delete_");
                return false;
            }
            ...
            if ((pos = file_contents.find(match_middle)) != std::string::npos) {
            ...
            } else if ((pos = file_contents.find(match_front)) !=
                       std::string::npos) {
                file_contents.replace(pos, match_front.length(), "[");
                fs->write_file(list_path, file_contents);
            } else if ((pos = file_contents.find(match_back)) !=
                       std::string::npos) {
                ...
                if (!fs->delete_file(list_path)) {
                    Logger::log_info(
                        "Error occurred while removing the list file");
                    return false;
                }
                // then delete directory
                if (!fs->delete_directory(directory_path)) {
                    Logger::log_info(
                        "Error occurred while removing the entity directory");
                    return false;
                }
            }

            // set response
            // delete request succeeded and client doesn't need to do anything
            // further, so return 204 status code
